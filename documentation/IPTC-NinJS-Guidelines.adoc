...
Added in ninjs 1.3, modified in ninjs 2.0. +
This property is an object with the following child properties: +
...
For a partial representation, for example a short extract provided as the results of an API call, the value `partial` should be used. (The value `incomplete` in 1.x was changed to `partial` in ninjs 2.0.)
|
One of: +
`complete` +
`partial`
...
Changed from `headline` and `headline_*` in ninjs 2.0. +
 +
This property is an object with the following child properties: +

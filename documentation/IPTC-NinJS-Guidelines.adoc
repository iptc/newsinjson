...
how easily a real ninjs document can be created. We have also built a JSON schema for
ninjs to help you <<How to validate objects,validate your ninjs implementation>>.

The latest version of the standard is 2.0: 
https://iptc.org/std/ninjs/ninjs-schema_2.0.json[Version 2.0 of the ninjs
schema] was made available in October 2021. An erratum release of version 2.0 was
published in May 2022 to fix a problem with the way GeoJSON was included in the
places object.
...
IPTC manages two current versions of ninjs: 1.4 and 2.0.
...
18th, 2022. All new users are strongly recommended to use version 2.0.
This document describes only the 2.0 version, but those who must use
version 1.4 for backwards compatibility reasons can find the JSON
Schema file for ninjs 1.4 schema at
https://iptc.org/std/ninjs/ninjs-schema_1.4.json.
...
Copyright © 2021 IPTC, International Press Telecommunications Council.
...
Here we document each property in a ninjs object. Only the "uri" property is required at this level. In some array objects other properties are required.
...
!version
!The version of the standard used, such as `2.0`.
!string

!schema
!The uri of the JSON schema to use for validation, such as `https://www.iptc.org/std/ninjs/ninjs-schema_2.0.json`
!URI
...
!name
!The name of the alternative id
!string
...
NOTE: See also `places` for places which are mentioned in the content.
|string
...
!geojson
!An object holding geo data of this place. As of ninjs 2.0, the contents of this
object must conform to https://geojson.org/schema/GeoJSON.json[the GeoJSON
format] defined in https://datatracker.ietf.org/doc/html/rfc7946[RFC 7946].
...
. change the URL of the "id" property of IPTC's ninjs 2.0 ...
+
[source,json]
{
...
[source,json]
{
  ...
  ... 
+
. add your own properties to your copy of the schema. For example, to add a property explaining the reason for the embargo:
+
[source,json]
{
  "$schema": "http://json-schema.org/draft-04/schema#",
  ...
  ...
+
. save your new schema to a location where users of the schema can access it, which should be the same URL used for the "id" property:
+
[source,json]
{
  "$schema": "http://json-schema.org/draft-04/schema#",
...
This is an example ninjs document that includes a reason for the embargo in an extended ninjs document:
...
https://iptc.org/std/ninjs/ninjs-schema_2.0.json
...
Version 2.0 made several changes, some of them breaking backwards compatibility:
...
* Added a new "rightsinfo" property with "linkedrights" or "embeddedrights" as child properties

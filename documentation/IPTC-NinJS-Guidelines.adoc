...
!contactInfo (contactinfo in 2.x)
...
!geoJSON (geojson in 2.x, geometry_<type> in 1.x)
...
!contactInfo (contactinfo in 1.x and 2.x)
...
[source,json]
----
include::includes/examples/008_event.json[]
----
...
NOTE: It's worth noting that this is different from the <<events>> metadata object used for news items. That object is intended to convey metadata about an event that is related to a published news item.
...
!===
!audience
1Intended audience(s) for the content.
!string
...
|wordCount": {
|The count of words of textual content. nar:wordcount",
|integer"
...
|n/a +
(3.0 only)
|The video encoding system used to create the content. Suggested CV: https://cv.iptc.org/newscodes/videocodec/
|string

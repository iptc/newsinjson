...
We have identified key properties and structures required to represent news and publishing information, and crafted JSON representations that are designed to result in lightweight instance documents. And we've built a JSON schema for ninjs to help you <<How to validate objects,validate your ninjs implementation>>.

The latest version of the standard is 1.2.

https://iptc.org/std/ninjs/ninjs-schema_1.2.json[Version 1.2 of the ninjs schema] was made available in October 2019.

Revision history:

https://iptc.org/std/ninjs/ninjs-schema_1.1.json[Version 1.1 of the ninjs schema] was made available in March 2014 and an error was corrected in September 2017.
...
The IPTC welcomes your feedback on how to improve the standard via the https://groups.io/g/iptc-ninjs[public ninjs discussion group].

You can also add and comment on issues in the
https://github.com/iptc/newsinjson[news in json github repository].
...
* ninjs is designed to be customisable by a particular provider, so that they can express just what they require, without unnecessary overhead. Ninjs is suitable for use in JSON-native engines, such ashttp://www.mongodb.org/[MongoDB] or
...
For a full document representation, the value should be `complete`. For a partial representation, for example a short extract provided as the results of an API call, the value `incomplete` should be used.
|
One of: +
`complete` +
`incomplete`
...
(`"$ref": "http://www.iptc.org/std/ninjs/ninjs-schema_1.2.json#"`) to point to
your new schema:
[source,json]
{
  "$schema": "http://json-schema.org/draft-04/schema#",
...
https://iptc.org/std/ninjs/ninjs-schema_1.2.json

...
how a real ninjs document can be. We have also built a JSON schema for
...
schema] was made available in October 2021.
...
!value
!The headline identified with the above role and contenttype. Value is required.
!string
...
!value
!The descriptive text identified with the above role and contenttype. Value is required.
!string
...
|===
|Property name|Description|Data type
|name|The name of this object in the array of renditions|string
...
The `associations` property contains associated valid ninjs objects, each with a name explaining the nature of the association.
...
https://iptc.org/std/ninjs/ninjs-schema_2.0.json[Version 2.0 of the ninjs schema] was approved by the IPTC Standards Committee and made available in XXXXXX.

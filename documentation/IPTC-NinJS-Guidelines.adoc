...
This property is an array of objects with no restrictions on key or value. +
Added in ninjs 1.3.
...
Version 1.3 added the `headline_`, `altids`, `trustindicator`, `$standard` and `genre` properties. It also changed the regular expression used for patternProperties used in `description_`, `body_`, `place`, `renditions`, `associations`, `geometry_` to ensure that the full string is being checked.
